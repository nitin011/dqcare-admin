...
class DoctorAuthController extends Controller
{
    ...
    public function verifyOtp(Request $request){
        ...
        $path = '/core/storage/app/public/backend/users/';
		...
		{
		$user['signature'] =  url('').$path.$user['signature'];
		}
    ...
    public function removeMyAccount(Request $request) {
        ...
        try {
            User::whereId(auth()->id())->delete();
            //UserSubscription::where('user_id', auth()->id())->delete();
            $user = Auth::user()->token();
            $user->revoke();
            return $this->successMessage('Doctor Account Removed Successfully!');
        } catch (\Exception $e) {
            $bug = $e->getMessage();
