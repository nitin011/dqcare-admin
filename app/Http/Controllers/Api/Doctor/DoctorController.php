...
class DoctorController extends Controller
{
    ...
    public function patientStore(Request $request)
    {
        ...
        try {
            ...
            if (!$phone) {
                ...
                callWhatsappNotification($user->phone, "Dr. " . auth()->user()->name . " added you in DQ Care. Now tracking family's health, managing medical documents and getting your health related updates easier than never before. Download DQ Care app today https://dqcare.in.");
            } else {
                return $this->errorOk('This patient is already using DQ Care. Please use patient QR code from app to add this patient.');
            }
    ...
    public function doctorSignature(Request $request)
    {
        // return $request->all();
        $this->validate($request, [
            'signature' => 'required',
        ]);
		$link = '/core/storage/app/public/backend/users/';
        try {
            $user = User::whereId(auth()->id())->first();
            if ($user) {
                if ($request->hasFile('signature')) {
                    ...
                    $path = storage_path() . '/app/public/backend/users/';
                    $imageName = 'Signature_' . $user->id . rand(000, 999) . '.' . $image->getClientOriginalExtension();
                    $image->move($path, $imageName);
                } 
                ...
                return $this->errorOk('The doctor\'s Signature can\'t be updated!');
            }
        } catch (\Exception $e) {
            return $this->error("Sorry! Failed to data! " . $e->getMessage());
        }
