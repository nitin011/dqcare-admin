...
class StoryController extends Controller
{
    ...
    public function storyOrganizeRequest(Request $request)
    {
        try {
            ...
            if(isset($storyRecord->updated_at) && $storyRecord->updated_at != null){
            ...
            if(isset($storyRecord->updated_at) && $storyRecord->updated_at){
	...
	public function patientSummaryUpdate(Request $request, $id)
    {
        try {
			
            $stories = Story::select('id', 'user_id','status', 'date', 'name', 'age', 'dob', 'detail','chart','journey', 'is_organize', 'type', 'created_at','remark')->with(['user' => function ($q) {
                $q->select('id', 'gender', 'first_name', 'phone', 'avatar', 'dob','updated_at');
            }])->where('user_id', $id)->where('type', 1)->latest()->first();
			
            if ($stories) {
                $stories['detail'] = json_decode($stories['detail'],1);
				
				$arr['detail']['known_cases'][0]['known_text'] = isset($stories['detail']['known_cases'][0]['known_text'])?$request->known_cases:'';
				$arr['detail']['family_history'][0]['family_text'] = isset($stories['detail']['family_history'][0]['family_text'])? $stories['detail']['family_history'][0]['family_text']:'';
				$arr['detail']['operative_history'][0]['operative_text'] = isset($stories['detail']['operative_history'][0]['operative_text'])?$stories['detail']['operative_history'][0]['operative_text']:'';
				$arr['detail']['habit_history'][0]['habit_text'] = isset($stories['detail']['habit_history'][0]['habit_text'])? $stories['detail']['habit_history'][0]['habit_text']:'';
				$arr['detail']['any_allergy'][0]['allergy_text'] = isset($stories['detail']['any_allergy'][0]['allergy_text'])?$request->any_allergy:'';
				$arr['detail']['current_medication'][0]['current_text'] = isset($stories['detail']['current_medication'][0]['current_text'])?$request->current_medication:'';
				$arr['detail']['vaccination_history'][0]['vaccination_history'] = isset($stories['detail']['vaccination_history'][0]['vaccination_history'])?$stories['detail']['vaccination_history'][0]['vaccination_history']:'';
				$arr['detail']['opd_visit'][0]['opd_text'] = isset($stories['detail']['opd_visit'][0]['opd_text'])?$stories['detail']['opd_visit'][0]['opd_text']:'';
				$arr['detail']['last_admitted'][0]['last_text'] = isset($stories['detail']['last_admitted'][0]['last_text'])? $stories['detail']['last_admitted'][0]['last_text']:'';
				$arr['detail']['bloodGroup'][0]['bloodGroup_text'] = isset($stories['detail']['bloodGroup'][0]['bloodGroup_text'])?$stories['detail']['bloodGroup'][0]['bloodGroup_text']:'';
				$arr['detail']['summary'] = isset($stories['detail']['summary']) ? $stories['detail']['summary']: '';
				$stories['detail'] = $arr['detail'];
			   ...
			   ->update([
				...
				$story = Story::create([
                    ...
                    'age' => $stories['age'],
                    'dob' => $stories['dob'],
                    'detail' => json_encode($stories['detail']),
                    'chart' => $stories['chart'],
                    'journey' => $stories['journey'],
                    'remark' => $stories['remark'],
                    'type' => $stories['type'],
                    'is_organize' => $stories['is_organize'],
                    'updated_by' => auth()->id()
                ]);
				
                return $this->successMessage('Summary Data Updated Successfully!');
            } else {
				$arr = [];
				$user = User::whereId($id)->first();
				
                $arr['detail']['known_cases'][0]['known_text'] = $request->known_cases;
				
				$arr['detail']['any_allergy'][0]['allergy_text'] =$request->any_allergy;
				$arr['detail']['current_medication'][0]['current_text'] = $request->current_medication;
